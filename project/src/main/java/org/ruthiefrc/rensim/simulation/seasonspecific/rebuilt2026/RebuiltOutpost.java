package org.ruthiefrc.rensim.simulation.seasonspecific.rebuilt2026;
...
import java.util.*;
import org.ruthiefrc.rensim.simulation.Goal;
...
public class RebuiltOutpost extends Goal {
    ...
    public RebuiltOutpost(Arena2026Rebuilt arena, boolean isBlue) {
        ...
        StructPublisher<Pose3d> OutpostPublisher = NetworkTableInstance.getDefault()
                .getStructTopic(
                        "/SmartDashboard/RenSim/Goals/" + (isBlue ? "BlueOutpost" : "RedOutpost"), Pose3d.struct)
                .publish();

        StructPublisher<Pose3d> OutpostThrowPublisher = NetworkTableInstance.getDefault()
                .getStructTopic(
                        "/SmartDashboard/RenSim/Goals/" + (isBlue ? "BlueOutpostThrow" : "RedOutpostThrow"),
                        Pose3d.struct)
                .publish();
        StructPublisher<Pose3d> OutpostDumpPublisher = NetworkTableInstance.getDefault()
                .getStructTopic(
                        "/SmartDashboard/RenSim/Goals/" + (isBlue ? "BlueOutpostDump" : "RedOutpostDump"),
                        Pose3d.struct)
                .publish();
